    ...
    <Faixa id="sobre" fundo="creme" className="py-16 sm:py-24">
      ...
      <ol className="mt-14 grid gap-0 border-t-2 border-[#3a271b] sm:grid-cols-2 lg:grid-cols-5">
        {MARCOS.map((m) => (
          ...
          >
            <div
              className="text-[13px] tracking-[0.14em] text-[#8c3a20]"
              style={{ fontFamily: '"Courier Prime", monospace' }}
            >
              {m.ano.toUpperCase()}
            </div>
            <h3 className="mt-2.5 text-[19px] leading-tight">{m.titulo}</h3>
            <p className="mt-2 text-[14px] leading-relaxed text-[#5c4635]">{m.texto}</p>
          </li>
        ))}
      </ol>

      <p className="ficha mt-8 max-w-[60ch] text-[12.5px] leading-relaxed text-[#8c7a66]">
